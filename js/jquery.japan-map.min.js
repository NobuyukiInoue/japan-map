...
(function($) {
    ...
    if (!('map' in Array.prototype)) {
        Array.prototype.map = function(a, b) {
            ...
            for (var i = 0, n = this.length; i < n; i++)
                if (i in this) c[i] = a.call(b, this[i], i, this);
            return c
        }
    }
    if (!('filter' in Array.prototype)) {
        Array.prototype.filter = function(a, b) {
            var c = [],
                v;
            for (var i = 0, n = this.length; i < n; i++)
                if (i in this && a.call(b, v = this[i], i, this)) c.push(v);
            return c
        }
    }
    var j = (function() {
    ...
    var l = j.Pointer ? 'pointerdown' : j.MSPointer ? 'MSPointerDown' : j.Touch ? 'touchstart' : 'mousedown';
    var m = j.Pointer ? 'pointermove' : j.MSPointer ? 'MSPointerMove' : j.Touch ? 'touchmove' : 'mousemove';
    var o = j.Pointer ? 'pointerup' : j.MSPointer ? 'MSPointerUp' : j.Touch ? 'touchend' : 'mouseup';
    var q = j.Pointer ? 'pointerenter' : j.MSPointer ? 'MSPointerEnter' : j.Touch ? 'touchenter' : 'mouseenter';
    var r = j.Pointer ? 'pointerleave' : j.MSPointer ? 'MSPointerLeave' : j.Touch ? 'touchleave' : 'mouseleave';
    var t = function(a) {
        this.options = a;
        this.base = {
            width: 651,
    ...
    t.prototype.setData = function(a, b) {
        this.data = {
            code: a ? a.code : null,
            name: a ? this.getName(a) : null,
            fullName: a ? a.name : null,
            ShortName: a ? this.getShortName(a) : null,
            englishName: a ? this.getEnglishName(a) : null,
            area: b ? b : null
        }
    };
    ...
    t.prototype.addEvent = function() {
        var b = this;
        var c = $(this.element);
        if (j.Pointer && !k || j.MSPointer && !k || j.Touch) {
            if (j.Pointer || j.MSPointer) {
                c.css("-ms-touch-action", "none").css("touch-action", "none")
            }
            c.on(l, function(e) {
                var a = e.originalEvent.changedTouches ? e.originalEvent.changedTouches[0] : e;
                b.pointer = {
                    x: a.pageX - c[0].offsetLeft,
                    y: a.pageY - c[0].offsetTop
                };
                b.render();
                if (b.isHovering()) {
                    e.preventDefault();
                ...
                c.on(m, function(e) {
                    a = e.originalEvent.changedTouches ? e.originalEvent.changedTouches[0] : e;
                    if (b.isHovering()) {
                        b.pointer = {
                            x: a.pageX - c[0].offsetLeft,
                            y: a.pageY - c[0].offsetTop
                ...
                $(document).on(o, function(e) {
                    a = e.originalEvent.changedTouches ? e.originalEvent.changedTouches[0] : e;
                    if (b.data.code !== null && b.data.name != null && "onSelect" in b.options) {
                        setTimeout(function() {
                            b.options.onSelect(b.data)
                        }, 0)
                    }
                    b.pointer = null;
                    c.off(m);
    ...
    t.prototype.findPrefectureByCode = function(a) {
        var b = this.options.prefectures.filter(function(p) {
            return p.code == a
        });
        return (b.length > 0) ? b[0] : null
    };
    t.prototype.findAreaBelongingToByCode = function(b) {
        var c = this.options.areas.filter(function(a) {
            return a.prefectures.indexOf(b) > -1
        });
        return (c.length > 0) ? c[0] : null
    };
    t.prototype.isNanseiIslands = function(a) {
        var b = ["屋久島", "種子島", "奄美諸島", "沖縄本島", "多良間島", "宮古島", "伊是名島", "伊平屋島", "八重山諸島"];
        return "name" in a && b.indexOf(a.name) > -1
    };
    t.prototype.brighten = function(a, b) {
        ...
        var d = "#",
            c, i;
        for (i = 0; i < 3; i++) {
            c = parseInt(a.substr(i * 2, 2), 16);
            c = Math.round(Math.min(Math.max(0, parseInt(c + (c * b))), 255)).toString(16);
            d += ("00" + c).substr(c.length)
        }
        return d
    };
    t.prototype.getName = function(a) {
        switch (this.isArea(a) ? this.options.areaNameType : this.options.prefectureNameType) {
            case "short":
                return this.getShortName(a);
            case "english":
                return this.getEnglishName(a);
            case "romaji":
                return this.getEnglishName(a);
            case "full":
                return a.name;
    ...
    t.prototype.getShortName = function(a) {
        if (this.isArea(a)) {
            return a.name.replace(/地方$/, "")
        }
        return a.name.replace(/[都|府|県]$/, "")
    };
    t.prototype.getEnglishName = function(a) {
        if (this.isArea(a)) {
            return a.english ? a.english : null
        }
        return A[a.code]
    };
    t.prototype.isArea = function(a) {
        return this.options.areas.indexOf(a) > -1
    };
    var u = function() {
    ...
    u.prototype.render = function() {
        var a = this.element.getContext("2d");
        a.clearRect(0, 0, this.element.width, this.element.height);
        this.hovering = false;
        this.hovered = null;
        var b = this.options.selection == "area" ? this.renderAreaMap : this.renderPrefectureMap;
        b.apply(this);
    ...
    u.prototype.renderPrefectureMap = function() {
        var c = this.element.getContext("2d");
        this.options.prefectures.forEach(function(a) {
            c.beginPath();
            this.drawPrefecture(a);
            c.closePath();
            var b = this.findAreaBelongingToByCode(a.code);
            if (b) {
            ...
            c.fill();
            if (this.options.borderLineColor && this.options.borderLineWidth > 0) c.stroke()
        }, this)
    };
    u.prototype.renderAreaMap = function() {
        var d = this.element.getContext("2d");
        this.options.areas.forEach(function(c) {
            d.beginPath();
            c.prefectures.forEach(function(a) {
                var b = this.findPrefectureByCode(a);
                if (b) {
                    this.drawPrefecture(b)
                } else {
                    throw "No prefecture code '" + a + "' is defined.";
                }
            }, this);
            d.closePath();
            this.setProperties(c, c);
            d.fill();
            if (this.options.borderLineColor && this.options.borderLineWidth > 0) d.stroke()
        }, this)
    };
    u.prototype.drawPrefecture = function(b) {
        b.path.forEach(function(p) {
            ...
            if (this.options.movesIslands) {
                a = {
                    X: a.X + (this.isNanseiIslands(p) ? this.NanseiIslands.left : -this.NanseiIslands.width),
                    Y: a.Y + (this.isNanseiIslands(p) ? this.NanseiIslands.top : 0)
                }
            }
            if ("coords" in p) this.drawCoords(p.coords, a);
            if ("subpath" in p) {
                p.subpath.forEach(function(s) {
                    if ("coords" in s) this.drawCoords(s.coords, a)
                }, this)
            }
        }, this)
    };
    u.prototype.drawName = function() {
        if (!this.options.showsPrefectureName && !this.options.showsAreaName) return;
        var f = this.options.showsAreaName && (!this.options.showsPrefectureName || this.options.selection == "area");
        if (f) {
            this.options.areas.forEach(function(d) {
                var e = {
                    x: 0,
                    y: 0,
                    n: 0
                };
                d.prefectures.forEach(function(a) {
                    var b = this.findPrefectureByCode(a);
                    var c = this.getCenterOfPrefecture(b);
                    e.n++;
                    e.x = (e.x * (e.n - 1) + c.x) / e.n;
                    e.y = (e.y * (e.n - 1) + c.y) / e.n
                }, this);
                this.drawText(d, e)
            }, this)
        } else {
            this.options.prefectures.forEach(function(a) {
                var b = this.getCenterOfPrefecture(a);
                this.drawText(a, b)
            }, this)
        }
    };
    u.prototype.drawText = function(a, b) {
        var c = this.element.getContext("2d");
        var d = this.isArea(a) ? a : this.findAreaBelongingToByCode(a.code);
        var e = this.options.showsAreaName && (!this.options.showsPrefectureName || this.options.selection == "area");
        c.save();
        if (this.options.fontColor && this.options.fontColor == "areaColor") {
            var f;
            if (e == (this.options.selection == "area")) {
                f = this.hovered == a.code
            } else if (e) {
                f = d.prefectures.indexOf(this.hovered) > -1
            } else {
                f = this.hovered == d.code
            }
            var g = d.color ? d.color : this.options.color;
            var h = d.color && d.hoverColor ? d.hoverColor : d.color ? this.brighten(d.color, 0.2) : this.options.hoverColor ? this.options.hoverColor : this.brighten(this.options.color, 0.2);
            c.fillStyle = f ? h : g
        ...
        c.font = (this.options.fontSize ? this.options.fontSize : this.element.width / 100) + "px '" + (this.options.font ? this.options.font : "Arial") + "'";
        c.textAlign = 'center';
        c.textBaseline = 'middle';
        if (this.options.fontShadowColor) {
            c.shadowColor = this.options.fontShadowColor;
            c.shadowBlur = 5
        }
        for (var i = 0; i < 5; i++) c.fillText(this.getName(a), b.x * this.element.width / this.base.width, b.y * this.element.height / this.base.height);
        c.restore()
    };
    u.prototype.getCenterOfPrefecture = function(a) {
        var b = {
            x: 0,
            y: 0,
            n: 0
        ...
        if (this.options.movesIslands) {
            c = {
                X: c.X + (this.isNanseiIslands(d) ? this.NanseiIslands.left : -this.NanseiIslands.width),
                Y: c.Y + (this.isNanseiIslands(d) ? this.NanseiIslands.top : 0)
            }
        }
        if ("coords" in d) {
            var i = 0;
            while (true) {
                var x = d.coords[i * 2 + 0];
                var y = d.coords[i * 2 + 1];
    ...
    u.prototype.drawCoords = function(a, b) {
        var c = this.element.getContext("2d");
        var i = 0;
        while (true) {
            var x = a[i * 2 + 0];
            ...
            y = y + b.Y;
            if (i == 0) {
                c.moveTo(x * this.element.width / this.base.width, y * this.element.height / this.base.height)
            } else {
                c.lineTo(x * this.element.width / this.base.width, y * this.element.height / this.base.height)
            }
            i++
        }
    };
    u.prototype.drawIslandsLine = function() {
        var a = this.element.getContext("2d");
        if (this.options.movesIslands) {
            a.beginPath();
            a.moveTo(0, 200 * this.element.height / this.base.height);
            a.lineTo(150 * this.element.width / this.base.width, 200 * this.element.height / this.base.height);
            a.lineTo(350 * this.element.width / this.base.width, 0);
            a.strokeStyle = this.options.lineColor;
            a.lineWidth = this.options.lineWidth;
            a.stroke()
        }
    };
    u.prototype.setProperties = function(a, b) {
        var c = this.element.getContext("2d");
        c.fillStyle = ("color" in b) ? b.color : this.options.color;
        if (this.options.borderLineColor) c.strokeStyle = this.options.borderLineColor;
        if (this.options.borderLineWidth) c.lineWidth = this.options.borderLineWidth;
        var d = this.pointer && c.isPointInPath(this.pointer.x, this.pointer.y);
        if (d) {
            this.hovering = true;
            this.hovered = a.code;
            if (this.data.code != a.code && this.options.onHover) {
                this.setData(a, b);
                this.options.onHover(this.data)
            } else {
                this.setData(a, b)
            }
            if (b.hoverColor) c.fillStyle = b.hoverColor;
            else if (this.options.hoverColor) c.fillStyle = this.options.hoverColor;
            else c.fillStyle = this.brighten(c.fillStyle, 0.2)
        }
        this.element.style.cursor = (this.data.code == null) ? "default" : "pointer"
    };
    u.prototype.isHovering = function() {
